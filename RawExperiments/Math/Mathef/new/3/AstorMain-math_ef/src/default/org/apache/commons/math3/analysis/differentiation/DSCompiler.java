...
public class DSCompiler {
	...
	private DSCompiler(final int parameters ,final int order ,final org.apache.commons.math3.analysis.differentiation.DSCompiler valueCompiler ,final org.apache.commons.math3.analysis.differentiation.DSCompiler derivativeCompiler) throws org.apache.commons.math3.exception.NumberIsTooLargeException {
	...
	public static org.apache.commons.math3.analysis.differentiation.DSCompiler getCompiler(int parameters, int order) throws org.apache.commons.math3.exception.NumberIsTooLargeException {
	...
	private static int[][][] compileMultiplicationIndirection(final int parameters, final int order, final org.apache.commons.math3.analysis.differentiation.DSCompiler valueCompiler, final org.apache.commons.math3.analysis.differentiation.DSCompiler derivativeCompiler, final int[] lowerIndirection) {
		...
		for (int i = 0 ; i < dSize ; ++i) {
			...
			java.util.List<int[]> row = new java.util.ArrayList<int[]>(((dRow.length) * 2));
	...
	private static int[][][] compileCompositionIndirection(final int parameters, final int order, final org.apache.commons.math3.analysis.differentiation.DSCompiler valueCompiler, final org.apache.commons.math3.analysis.differentiation.DSCompiler derivativeCompiler, final int[][] sizes, final int[][] derivativesIndirection) throws org.apache.commons.math3.exception.NumberIsTooLargeException {
		if ((parameters == 0) || (order == 0)) {
	...
	private static int convertIndex(final int index, final int srcP, final int[][] srcDerivativesIndirection, final int destP, final int destO, final int[][] destSizes) throws org.apache.commons.math3.exception.NumberIsTooLargeException {
	...
	public void remainder(final double[] lhs, final int lhsOffset, final double[] rhs, final int rhsOffset, final double[] result, final int resultOffset) {
		final double rem = org.apache.commons.math3.util.FastMath.IEEEremainder(lhs[lhsOffset], rhs[rhsOffset]);
	...
	public void pow(final double a, final double[] operand, final int operandOffset, final double[] result, final int resultOffset) {
		...
		if (a == 0) {
			if ((operand[operandOffset]) == 0) {
				...
				java.util.Arrays.fill(function, java.lang.Double.NaN);
			} 
		} else {
			function[0] = org.apache.commons.math3.util.FastMath.pow(a, operand[operandOffset]);
			final double lnA = org.apache.commons.math3.util.FastMath.log(a);
			for (int i = 1 ; i < (function.length) ; ++i) {
	...
	public double taylor(final double[] ds, final int dsOffset, final double... delta) throws org.apache.commons.math3.exception.MathArithmeticException {
		double value = 0;
		for (int i = (getSize()) - 1 ; i >= 0 ; --i) {
			...
			for (int k = 0 ; k < (orders.length) ; ++k) {
				if ((orders[k]) > 0) {
					try {
						term *= (org.apache.commons.math3.util.FastMath.pow(delta[k], orders[k])) / (org.apache.commons.math3.util.CombinatoricsUtils.factorial(orders[k]));
