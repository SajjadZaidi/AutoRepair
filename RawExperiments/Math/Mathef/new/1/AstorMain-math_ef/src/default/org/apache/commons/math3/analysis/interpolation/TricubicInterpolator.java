...
public class TricubicInterpolator implements org.apache.commons.math3.analysis.interpolation.TrivariateGridInterpolator {
	public org.apache.commons.math3.analysis.interpolation.TricubicInterpolatingFunction interpolate(final double[] xval, final double[] yval, final double[] zval, final double[][][] fval) throws org.apache.commons.math3.exception.DimensionMismatchException, org.apache.commons.math3.exception.NoDataException, org.apache.commons.math3.exception.NonMonotonicSequenceException, org.apache.commons.math3.exception.NumberIsTooSmallException {
		...
		for (int i = 1 ; i < (xLen - 1) ; i++) {
			if ((yval.length) != (fval[i].length)) {
				throw new org.apache.commons.math3.exception.DimensionMismatchException(yval.length , fval[i].length);
			} 
			final int nI = i + 1;
			final int pI = i - 1;
			...
			for (int j = 1 ; j < (yLen - 1) ; j++) {
				if ((zval.length) != (fval[i][j].length)) {
					throw new org.apache.commons.math3.exception.DimensionMismatchException(zval.length , fval[i][j].length);
				} 
				final int nJ = j + 1;
				final int pJ = j - 1;
		...
		return new org.apache.commons.math3.analysis.interpolation.TricubicInterpolatingFunction(xval, yval, zval, fval, dFdX, dFdY, dFdZ, d2FdXdY, d2FdXdZ, d2FdYdZ, d3FdXdYdZ) {
			@java.lang.Override
