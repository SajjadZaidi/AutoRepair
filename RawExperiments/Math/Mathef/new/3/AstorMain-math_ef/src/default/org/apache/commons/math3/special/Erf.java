...
public class Erf {
	...
	public static double erfInv(final double x) {
		double w = -(org.apache.commons.math3.util.FastMath.log(((1.0 - x) * (1.0 + x))));
		double p;
		if (w < 6.25) {
			w -= 3.125;
			...
			w = (org.apache.commons.math3.util.FastMath.sqrt(w)) - 3.25;
		...
		} else if (!(java.lang.Double.isInfinite(w))) {
			w = (org.apache.commons.math3.util.FastMath.sqrt(w)) - 5.0;
			...
			p = java.lang.Double.POSITIVE_INFINITY;
		}
		return p * x;
	}

	public static double erfcInv(final double x) {
		return org.apache.commons.math3.special.Erf.erfInv((1 - x));
	}
}
