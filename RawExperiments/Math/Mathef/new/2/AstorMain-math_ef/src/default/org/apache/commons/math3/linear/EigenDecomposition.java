...
public class EigenDecomposition {
	...
	public org.apache.commons.math3.linear.RealMatrix getD() {
		if ((cachedD) == null) {
			...
			for (int i = 0 ; i < (imagEigenvalues.length) ; i++) {
				...
				} else if ((org.apache.commons.math3.util.Precision.compareTo(imagEigenvalues[i], 0.0, org.apache.commons.math3.linear.EigenDecomposition.EPSILON)) < 0) {
					cachedD.setEntry(i, (i - 1), imagEigenvalues[i]);
				} 
			}
		} 
	...
	private static class Solver implements org.apache.commons.math3.linear.DecompositionSolver {
		...
		public boolean isNonSingular() {
			...
			for (int i = 0 ; i < (realEigenvalues.length) ; ++i) {
				largestEigenvalueNorm = org.apache.commons.math3.util.FastMath.max(largestEigenvalueNorm, eigenvalueNorm(i));
			}
			if (largestEigenvalueNorm == 0.0) {
				return false;
			} 
			for (int i = 0 ; i < (realEigenvalues.length) ; ++i) {
				if (org.apache.commons.math3.util.Precision.equals(((eigenvalueNorm(i)) / largestEigenvalueNorm), 0, org.apache.commons.math3.linear.EigenDecomposition.EPSILON)) {
					return false;
				} 
			}
			return true;
		...
		private double eigenvalueNorm(int i) {
			...
			return org.apache.commons.math3.util.FastMath.sqrt(((re * re) + (im * im)));
		}
